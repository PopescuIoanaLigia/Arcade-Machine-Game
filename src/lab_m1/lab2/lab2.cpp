...
void Lab2::Init()
{
    ...
    // by definition!
    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(1, -1,  -1), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(-1, 1,  -1), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(-1.366f, -1, -1.366f), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
        };
...
void Lab2::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
    ...
    meshes["cube_B"] = new Mesh("cube_B");
    meshes["cube_B"]->InitFromBuffer(VAO, static_cast<unsigned int>(indices.size()));
}
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["tetraedru"], shaders["VertexColor"], glm::vec3(0, 1.5f, 0), glm::vec3(0.25f));
